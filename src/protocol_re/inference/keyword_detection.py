...
from protocol_re.config.thresholds import KeywordDetection as _KD
...
def find_keyword_byte(messages_hex: Sequence[str], search_range: range = range(_KD.SEARCH_RANGE_START, _KD.SEARCH_RANGE_END)) -> Optional[Dict[str, float]]:
...
def split_family_by_keyword(messages_hex: Sequence[str], search_range: range = range(_KD.SEARCH_RANGE_START, _KD.SEARCH_RANGE_END)) -> Dict[str, object]:
