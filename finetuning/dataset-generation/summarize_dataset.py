...
def main():
    ...
    tok=AutoTokenizer.from_pretrained(a.tokenizer); protocols=Counter(); tasks=Counter(); families=Counter(); prompt_tokens=[]; target_tokens=[]; fields=[]; boundaries=[]; records=0
    for path in a.input:
        for line_no,line in enumerate(path.read_text(encoding="utf-8").splitlines(),1):
            ...
            messages=row["messages"]; rendered=tok.apply_chat_template(messages[:-1],tokenize=False,add_generation_prompt=True); target=messages[-1]["content"]
