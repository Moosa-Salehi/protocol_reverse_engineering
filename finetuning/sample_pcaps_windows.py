...
def main()->None:
    ...
    a=p.parse_args();cfg=json.loads(a.protocols.read_text(encoding="utf-8"));selected=dict(cfg["train"]);selected.update(cfg["holdout"] if a.include_holdout else {});files=sorted(x for x in a.pcap_dir.rglob("*") if x.is_file() and x.suffix.lower() in SUFFIXES);a.output_dir.mkdir(parents=True,exist_ok=True);rng=random.Random(a.seed);report={"pcap_root":str(a.pcap_dir),"protocols":{}}
    ...
    for protocol,spec in selected.items():
        ...
        allocations=allocate([len(x[1]) for x in matches],a.budget_per_protocol,a.min_per_file,a.max_per_file);outdir=a.output_dir/protocol;outdir.mkdir(parents=True,exist_ok=True);items=[]
