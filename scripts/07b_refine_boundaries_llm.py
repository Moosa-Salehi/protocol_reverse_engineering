...
def main() -> None:
    ...
    for family_id, details in families_data.items():
        ...
        # Run boundary refinement stage. The local backend replaces the whole
        # prompt+call: it renders training-format per-message prompts itself.
        if local_llm is not None:
            def local_call(prompt: str, family_id: str = family_id, task: str = "boundary_refinement") -> str:
                raw, _boundaries, _used = run_local_boundary_refinement(
                    family_id, sample_messages, local_llm
                )
                return raw
