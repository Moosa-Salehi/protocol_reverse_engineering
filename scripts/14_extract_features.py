...
from protocol_re.features.extraction import extract_feature_artifacts
...
def main() -> None:
    ...
    message_features, family_features = extract_feature_artifacts(
        records,
    ...
    with open(message_path, "w", encoding="utf-8") as handle:
        for record in message_features:
            handle.write(json.dumps(record, sort_keys=True) + "\n")

    with open(family_path, "w", encoding="utf-8") as handle:
        json.dump(family_features, handle, indent=2)

    print(f"[+] Wrote {len(message_features)} message feature records to {message_path}")
